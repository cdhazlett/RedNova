...
def printOutput(Ephemeris, EphParams, Star, BJD, cycles, phase):
    """For testing purposes, write the computed values to an output file."""
    # Open file
    outString = ""
    # Write header with information about run
    outString += 'OrbitalPhasePackage\n'+Star+'\n'
    if Ephemeris == 'Exponential':
        outString += 'Exponential\nT1,T0,Period (d),tau (d),P_ref (d)\n'
        outString += '%.6f,%.1f,%.9f,%.9f,%.9f\n' % (T1,T0,P0,tau,P1)
    elif Ephemeris == 'Quadratic':
        outString += 'Quadratic\nT1,Period (h),Pdot\n'
        outString += '%.6f,%.9f,%.3e\n' % (EphParams[0],EphParams[1],EphParams[2])
    elif Ephemeris == 'Linear':
        ...
        outString += '%.6f,%.9f,%.3e\n' % (EphParams[0],EphParams[1])
    # Write out results
    outString += 'BJD,cycles,Phase\nEOH\n'
    for i in range(len(BJD)):
        outString += '{0:13.5f},{1:10.5f},{2:.4f}\n'.format(BJD[i],cycles[i],phase[i])
    # Close file
    # ofile.close()
    print outString
    return
...
if __name__ == '__main__':
    ...
    printOutput(Ephemeris, EphParams, Star, BJD, cycles, phase)
